...
>>>>>>> a400078764b98522fee96ded515f61837496b4c4
			...
			//{
				...
				WorldIsTin = WorldGen.SavedOreTiers.Copper != TileID.Copper;
				int x = 0;
		...
		*/
		...
		public override void PostUpdateWorld()
		{
			...
			//if ((Main.netMode < NetmodeID.MultiplayerClient || Main.myPlayer == 0) && Main.expertMode)
			//{
			...
			/*if (NPC.CountNPCS(NPCID.Golem) > 0 && SGAConfig.Instance.GolemImprovement)
			{
				...
				if (NPC.CountNPCS(Mod.Find<ModNPC>("SGAGolemBoss").Type) < 1)
				{
					...
					NPC.NewNPC((int)myowner.position.X, (int)myowner.position.Y, Mod.Find<ModNPC>("SGAGolemBoss").Type);
			...
			if (questvars[10] > 2400 && questvars[10] < 3750)
			{
				...
				{
					SoundEngine.PlaySound(SoundID.Zombie105.WithPitchOffset(-0.6f));
...
>>>>>>> a400078764b98522fee96ded515f61837496b4c4
		...
		*/
		...
		public static void GenVirulent()
		{
			...
			if (Main.netMode == NetmodeID.MultiplayerClient || WorldGen.noTileActions || WorldGen.gen || !Main.hardMode || GennedVirulent)
			{
				return;
			}
		...
		*/
		...
		{
			int tiletype2 = ModContent.TileType<Tiles.Bars.NovusOreTile>();
			if (WorldGen.genRand.NextBool())
			{
				WorldIsNovus = false;
				tiletype2 = ModContent.TileType<Tiles.Bars.NoviteOreTile>();
			}
			...
			for (double k = 0; k < (Main.maxTilesX - 200) * (Main.maxTilesY - 150 - (int)Main.rockLayer) / 25.0 / 1.0; k += 1.0)
			{
				...
				if (tile.HasTile && (tile.TileType == TileID.Dirt || tile.TileType == TileID.Stone || tile.TileType == TileID.RainCloud || tile.TileType == TileID.Cloud))
				{
					...
					{
						chance = 3;
						tiletype = ModContent.TileType<Tiles.Bars.Photosyte>();
						size[0] = 2;
						size[1] = 5;
					}
					bool randon = (WorldGen.genRand.Next(0, 1000) < 3);
					if (tile.HasTile && (geny < GenVars.worldSurfaceLow || (randon)))
					{
		...
		public static int RaycastDownWater(int x, int y, int check)
		{
			...
			while (Main.tile[x, y].LiquidAmount < check && y < Main.maxTilesY - 5)
			{
				y++;
			}
			return y;
		...
		public override void TileCountsAvailable(ReadOnlySpan<int> tileCounts)
		{
			...
			//MoistStonecount = tileCounts[ModContent.TileType<MoistStone>()]; #TODO
		}
		...
		*/
		...
		{
			oreTypesPreHardmode[0] = WorldGen.SavedOreTiers.Copper;
			oreTypesPreHardmode[1] = WorldGen.SavedOreTiers.Iron;
			oreTypesPreHardmode[2] = WorldGen.SavedOreTiers.Silver;
			oreTypesPreHardmode[3] = WorldGen.SavedOreTiers.Gold;

			oreTypesHardmode[0] = WorldGen.SavedOreTiers.Cobalt;
			oreTypesHardmode[1] = WorldGen.SavedOreTiers.Mythril;
			oreTypesHardmode[2] = WorldGen.SavedOreTiers.Adamantite;
